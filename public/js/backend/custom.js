...
function chooseUser(e,id, name, phone){
	...
	if (client_phone !== null) {phone != 'null' ? client_phone.value = phone : client_phone.value = '';};
...
function reciveTransfer(e ,id ,value , amount, company, note, mother_name, birthday){
	
	$('#transfer-id-input').val(id);
	$('#form-action').attr('action',window.location.origin+'/admin/transfers/recive/'+id);
...
(function () {
	...
	function(){
        ...
        getData(window.location.origin+'/api/v1/search/clients/'+client_name)
		...
		.then((data) => {
			...
			$.each(data['data'], function(index,value) {
			  ...
			  <div class='col-7'>\
				<label>الاسم: "+ value['name']+"</label>\
			</div>\
			<div class='col-5'>\
				<label>موبايل: "+ value['phone']+"</label>\
			</div>\
			<div class='col-7'>\
				<label>اسم الام : "+ value['mother_name']+"</label>\
			</div>\
			<div class='col-5'>\
				<label>تاريخ الميلاد : "+ value['birthday']+"</label>\
			</div>\
			</div>\
			<div class='col-1'>\
			<button id='choose_user"+value['id']+"' name='btn-choose' onclick='chooseUser(this,"+value['id']+",\""+value['name']+"\",\""+value['phone']+"\")' type='button' class='btn btn-success'>اختيار</button>\
			</div>\
			</div>"
			});
			str += "</div>";
	...
	$('#btn-search-transfer').on('click',function(){
		...
		getData(window.location.origin+'/api/v1/search/transfers/clients/'+id)
		.then(response => response.json())
		.then((data) => {
			console.log(data);
			...
			$.each(data['data'], function(index,value){
			  ...
			  <div class='col-11 row'>\
			  <div class='col-7'>\
				<label><b>معرف الحوالة:</b> "+value['id']+" </label>\
			</div>\
			<div class='col-12'>\
				<label><b>تاريخ الحوالة:</b> "+value['created_at']+" </label>\
			</div>\
			<div class='col-12'>\
				<label><b>اسم الزبون :</b> "+value['client']+"  </label>\
			</div>\
			<div class='col-7'>\
				<label><b>قيمة الحوالة:</b> "+value['amount']+" </label>\
			</div>\
			<div class='col-5'>\
				<label><b>الشركة:</b> "+value['company']+" </label>\
			</div>\
			<div class='col-12'>\
				<label><b>ملاحظة:</b> "+value['note']+" </label>\
			</div>\
			</div>\
			<div class='col-1'>\
			<form action='/transfers/"+value['id']+"' method='PUT'>\
			<input name='status' type='text' value='T' hidden></input>\
			<button id='choose_user' name='btn-choose' data-toggle='modal' data-target='#confirm-popup' onclick='reciveTransfer(this,\""+value['id']+"\",\""+value['client']+"\",\""+value['amount']+"\",\""+value['company']+"\",\""+value['note']+"\",\""+value['mother_name']+"\",\""+value['birthday']+"\")' type='button' class='btn btn-success'>تسليم الحوالة</button>\
			</form>\
			</div>\
			</div>"
			});
